...
| --------------------------------- | ---- | ---- | ---- | ------ | ---- | -------- | -------------------------------------------------------------------- |
| claude-sonnet-4-6                 | 👑   | 🥈   | 🥇   | 🥈     | 👁️   | ❌       | Anthropic 当前推荐主力模型，综合能力强，适合主对话模型                 |
| claude-opus-4-6                   | 👑   | 🥉   | 🥇   | 🥉     | 👁️   | ❌       | Anthropic 高质量模型，适合复杂推理、长任务和高质量输出场景             |
| gemini-3.1-pro-preview            | 👑   | 🥈   | 🥇   | 🥉     | 👁️   | 🧠       | Gemini 高质量主力模型，适合复杂任务 ⚠️ 预览版模型                      |
| gemini-3.5-flash                  | 🥇   | 🥇   | 🥇   | 🥇     | 👁️   | ❌       | Gemini Flash 系列较新型号，适合作为高性价比主力模型                    |
| gemini-3.1-flash-lite-preview     | 🥈   | 👑   | 🥇   | 👑     | 👁️   | ❌       | 极速小模型，推理成本低，适合简单任务或快速迭代 ⚠️ 预览版模型            |
| gpt-5.5                           | 🥇   | 🥈   | 🥇   | 🥈     | 👁️   | ❌       | GPT 系列较新主力模型，适合通用生产力场景                               |
| gpt-5.4-mini                      | 🥈   | 🥇   | 🥇   | 🥇     | 👁️   | ❌       | GPT 系列小模型，适合低成本快速回复                                     |
| MiniMax-M2.7                      | 🥇   | 🥈   | 🥇   | 🥇     | ❌   | ❌       | MiniMax 系列较新模型，中文对话和通用任务表现均衡                       |
| deepseek-v3                       | 🥇   | 🥉   | 🥇   | 🥈     | ❌   | ❌       | DeepSeek V3 系列通用聊天模型，中文能力优秀                             |
| deepseek-reasoner                 | 🥇   | 🥉   | 🥇   | 🥈     | ❌   | 🧠       | DeepSeek 推理模型，适合需要更强逻辑推演的任务                           |
| doubao-1.5-vision-pro-32k-250115  | 🥈   | 🥈   | 👑   | 🥈     | 👁️   | ❌       | 字节提供的国产视觉模型，稳定性好，适合作为多模态备用模型                |
| qwen-plus-latest                  | 🥈   | 🥇   | 🥇   | 🥇     | ❌   | ❌       | Qwen Plus 系列较新模型，适合中文日常对话和低成本任务                    |
| qwen-max-latest                   | 🥇   | 🥈   | 🥇   | 🥈     | ❌   | ❌       | Qwen Max 系列较新模型，适合更高质量的中文任务                           |
| grok-3                            | 🥈   | 🥈   | 🥇   | 🥉     | 👁️   | ❌       | xAI 推出的语言模型，个性鲜明，适合风格化对话                            |
...
| --------------------------------- | ---- | ---- | ---- | ------ | ---- | ---- | ---------------------------------------- |
| claude-opus-4-6                   | 👑   | 🥉   | 🥇   | 🥉     | 👁️   | 🧠   | Anthropic 高质量模型，适合复杂插件开发和重构任务 |
| claude-sonnet-4-6                 | 👑   | 🥈   | 🥇   | 🥈     | 👁️   | 🧠   | Anthropic 当前推荐主力编码模型，质量和速度较均衡 |
| gemini-3.1-pro-preview            | 👑   | 🥈   | 🥇   | 🥉     | 👁️   | 🧠   | Gemini 高质量主力模型，编程领域表现较好 ⚠️ 预览版模型 |

在生成模型生成修改建议后，我们还需要通过 `插件代码应用模型组(PLUGIN_APPLY_MODEL_GROUP)` 来在当前插件编辑器中应用修改建议，推荐使用强提示词遵循能力、生成速度快的模型，以下是推荐模型列表:

| 模型名称         | 质量 | 速度 | 稳定 | 性价比 | 视觉 | 思维 | 备注 |
| ---------------- | ---- | ---- | ---- | ------ | ---- | ---- | ---- |
| gemini-3.5-flash       | 🥇   | 👑   | 🥇   | 👑     | 👁️   | ❌   | 推荐的快速逻辑应用模型 |
| claude-haiku-4-5       | 🥈   | 🥇   | 🥇   | 🥈     | 👁️   | ❌   | Anthropic 快速模型，适合简单代码应用和格式修正 |
...
| ------------------------------ | ---- | ---- | ---- | ------ | ------ | ------------ | -------------------------------------------------- |
| gemini-3.1-flash-image-preview | 👑   | 🥇   | 🥇   | 🥇     | ✅     | 聊天模式     | Gemini 3.1 绘图模型，具备较好的理解能力与视觉质量   |
| gpt-image-2                    | 👑   | 🥈   | 🥇   | 🥈     | ✅     | 图像生成模式 | OpenAI 较新的图像模型，适合高质量绘图任务           |
| sora_image                     | 🥇   | ⚪   | 🥇   | 🥈     | ✅     | 聊天模式     | 与 ChatGPT 官网绘图体验接近，逻辑遵循好但速度较慢   |
