...
dynamic_import_pkg(
    ...
    *,                                    # 以下为 keyword-only 参数
    mirror: Optional[str] = None,         # PyPI 镜像源，默认从配置读取
...
默认从配置项 `DYNAMIC_PLUGIN_INSTALL_MIRROR` 读取镜像源，也可以在调用时指定：
...
# RuntimeError: 安装超时（内部捕获 subprocess.TimeoutExpired 后重新抛出）
# - 包体积过大
# - 网络速度过慢
```
...
通过合理使用动态包导入功能，您的插件可以灵活地利用 Python 丰富的生态系统，实现更加强大和多样化的功能。
