...
在您的插件主文件 (例如 `plugin.py`) 中，首先需要导入并创建一个 `NekroPlugin` 实例：
...
plugin = NekroPlugin(
    ...
    is_package=False,  # 如果您的插件是一个复杂的包结构，可能需要调整
...
插件的生命周期描述了从插件被系统发现、加载、运行到最终卸载的整个过程。Nekro Agent 提供了一系列装饰器，允许您在生命周期的特定阶段执行自定义逻辑。
        ...
        if collection_name not in collection_names:
            ...
            await client.create_collection(
                ...
                vectors_config=qdrant_models.VectorParams(
                    size=768,  # 示例维度，根据您的 embedding 模型调整
...
async def handle_user_message(_ctx: AgentCtx, message: ChatMessage) -> MsgSignal | None:
    ...
    if "禁止词" in message.content_text:
        core.logger.warning(f"用户 {_ctx.from_platform_userid} 发送了包含禁止词的消息")
    ...
    await plugin.store.set(
        ...
        value=str(int(await plugin.store.get(user_key=_ctx.from_platform_userid, store_key="message_count") or "0") + 1)
    )
...
- `MsgSignal.FORCE_TRIGGER`: 强制触发后续处理（即使消息原本不会触发 AI）
...
插件可以通过 `plugin.get_config()` 方法访问其配置：
...
通过合理使用这些特性，您可以创建出功能强大、用户友好的 Nekro Agent 插件。

### 插件启用/禁用回调 (`@plugin.on_enabled()` / `@plugin.on_disabled()`)

插件可以在状态变更时注册回调函数，分别在插件被启用或禁用后立即执行。
...
async def handle_enabled():
    plugin.logger.info(f"插件 '{plugin.name}' 已启用")

@plugin.on_disabled()
async def handle_disabled():
    plugin.logger.info(f"插件 '{plugin.name}' 已禁用")
```

### Webhook 方法 (`@plugin.mount_webhook_method()`)

插件可以注册 Webhook 方法，通过 HTTP 请求触发插件功能。

**参数**：
- `endpoint` (str): Webhook 端点名称
- `name` (str): 方法名称
...
```python
@plugin.mount_webhook_method(endpoint="notify", name="接收通知", description="处理外部系统推送的通知")
async def handle_webhook(_ctx: AgentCtx, data: dict) -> str:
    plugin.logger.info(f"收到 Webhook 通知: {data}")
    return "通知已处理"
```

### 自定义路由 (`@plugin.mount_router()`)

插件可以挂载自定义的 FastAPI 路由，挂载在 `/plugins/{plugin_key}` 路径下。
...
**注意**：通过 `mount_router` 添加的路由在运行时无法移除（FastAPI 限制），只能标记为已卸载。

### 异步任务 (`@plugin.mount_async_task()`)

插件可以注册长时间运行的异步任务，支持进度追踪、暂停等待外部信号、取消等。

**参数**：
- `task_type` (str): 任务类型标识，用于启动和查询任务

**示例**：

```python
from nekro_agent.api.plugin import AsyncTaskHandle, TaskCtl

@plugin.mount_async_task("video_gen")
async def video_generation(handle: AsyncTaskHandle, prompt: str):
    yield TaskCtl.report_progress("正在生成视频...")
    # 执行耗时操作
    result = await generate_video(prompt)
    yield TaskCtl.report_progress("等待审批")
    approved = await handle.wait("approval", timeout=3600)
    if approved:
        yield TaskCtl.success("视频生成完成", data=result)
    else:
        yield TaskCtl.fail("审批被拒绝")
```

### 插件命令 (`@plugin.mount_command()` / `@plugin.mount_command_group()`)

插件可以注册聊天命令，用户通过 `/命令名 参数` 的方式调用。

**mount_command 参数**：
- `name` (str): 命令名
- `description` (str): 命令描述
- `aliases` (List[str], 可选): 别名列表
- `permission` (CommandPermission, 可选): 权限级别，默认 `PUBLIC`
- `usage` (str, 可选): 使用说明
- `category` (str, 可选): 分类，默认 `"plugin"`
- `tags` (List[str], 可选): 标签，便于 Agent 检索
- `internal` (bool, 可选): 是否为内部命令（不在帮助列表中显示）
- `i18n_description` / `i18n_usage` / `i18n_category` (Dict[str, str], 可选): 国际化字段
...
async def weather_command(
    ...
    location: str = Arg("城市名称", default="北京", positional=True),
) -> CommandResponse:
    weather = await get_weather(location)
    return CmdCtl.success(f"{location} 当前天气：{weather}")
```
...
async def config_set(context, key: str = Arg("配置键", positional=True), value: str = Arg("配置值", positional=True)):
    return CmdCtl.success(f"已设置 {key} = {value}")

# 调用: /config.set key:"model" value:"gpt-4"
```

### 插件数据目录 (`plugin.get_plugin_data_dir()`)

获取插件专属的数据存储目录，用于存储插件的运行时数据、日志、缓存等。目录位于 `DATA_DIR/plugin_data/{author}.{module_name}/`。
...
### 保存配置 (`plugin.save_config()`)

当需要在运行时修改并持久化插件配置时，使用 `save_config()` 方法：
