...
*   `json_schema_extra`: 用于传递额外的 WebUI 控制参数。现在推荐使用 `ExtraField` 模型来生成，而不是手动构建字典。`ExtraField` 提供以下属性：
    ...
    *   `ref_email_accounts_send_enabled` (bool): 引用已启用发信的邮箱账户选择器
    *   `ref_presets` (bool): 引用人设选择器（包含默认人设）
    *   `ref_presets_no_default` (bool): 引用人设选择器（不包含默认人设）
    *   `ref_presets_multiple` (bool): 引用人设多选选择器
    *   `i18n_category` (Dict[str, str]): 配置项分类的国际化映射
    *   `i18n_title` (Dict[str, str]): 配置项标题的国际化映射
    *   `i18n_description` (Dict[str, str]): 配置项描述的国际化映射
...
一旦配置类被注册，您就可以在插件的任何地方通过 `plugin.get_config()` 方法访问到配置类的实例，并进而访问各个配置项。
...
5.  **配置热更新**：Nekro Agent 通常支持配置的即时生效，无需重启插件或 Agent。在插件代码中访问 `plugin.get_config()` 时，获取到的通常就是最新的配置值。

通过精心设计插件配置，您可以极大地提升插件的易用性、灵活性和安全性。