...
为了确保 AI 能够正确、高效地使用您的沙盒方法，请遵循以下规范：
...
2.  **详细且结构化的文档字符串 (Docstring)**：
    这是 AI 理解和使用您的沙盒方法的最重要信息来源之一！文档字符串应该遵循一定的格式（如 Google 风格、Numpy 风格），并至少包含：
    *   **简洁的摘要行**：清晰说明方法的功能。
...
6.  **上下文 (`AgentCtx`) 的正确使用**：
    `AgentCtx` 对象 (`_ctx`) 包含了当前会话的关键信息，如 `_ctx.from_chat_key` (会话标识)、`_ctx.from_platform_userid` (用户标识)等。按需使用这些信息，例如用于区分不同会话的数据存储、获取消息发送者等。

通过遵循这些规范，您可以创建出 AI 更易于理解、更乐于使用、也更不容易出错的强大沙盒方法。