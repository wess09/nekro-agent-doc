...
### プラグインロガー（`plugin.logger`）

各プラグインインスタンスには **`plugin.logger`** が用意されており、プラグイン内のログ出力に使います。グローバルな `core.logger` の代わりに `plugin.logger` の利用を推奨します。

*   **プラグインでフィルタ**：ログに `plugin_key`（例：`Author.module_name`）が付与され、管理者は WebUI の「ログ」ページでプラグイン別に絞り込めます。
*   **標準 logger と同じ API**：`debug`、`info`、`warning`、`error`、`exception` などをサポート。例：
    *   `plugin.logger.info("タスクを作成しました")`
    *   `plugin.logger.exception("処理に失敗しました")`  # スタックトレース付き

```python
# 推奨：マウントしたメソッド、初期化、Webhook などで使用
plugin.logger.info(f"プラグイン '{plugin.name}' を初期化しています...")
plugin.logger.warning("設定がありません。デフォルトを使用します")
plugin.logger.error(f"リクエスト失敗: {e}")
```
