---
title: データストレージ
description: "Nekro Agentプラグインの2つのストレージ方法の使用方法を学習します：KVキー値ストレージ（plugin.store）とプラグイン永続ディレクトリ（plugin.get_plugin_path()）、使用例、ベストプラクティス、完全な例を含みます"
---
...
`plugin.get_plugin_path()`は、プラグイン専用のファイルシステムディレクトリを指す`pathlib.Path`オブジェクトを返します。各プラグインは独立したディレクトリを持ち、通常は以下の場所にあります：
