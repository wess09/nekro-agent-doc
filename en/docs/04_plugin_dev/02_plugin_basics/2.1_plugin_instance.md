...
Each plugin instance has a **`plugin.logger`** for logging inside the plugin. Prefer using `plugin.logger` instead of the global `core.logger`:

*   **Filter by plugin**: Logs are tagged with `plugin_key` (e.g. `Author.module_name`), so admins can filter by plugin on the WebUI Logs page.
*   **Same API as a standard logger**: Supports `debug`, `info`, `warning`, `error`, `exception`, etc., e.g.:
    *   `plugin.logger.info("Task created")`
    *   `plugin.logger.exception("Processing failed")`  # includes traceback

```python
# Recommended: use in mounted methods, init, webhooks, etc.
plugin.logger.info(f"Plugin '{plugin.name}' is initializing...")
plugin.logger.warning("Config missing, using default")
plugin.logger.error(f"Request failed: {e}")
```
...
async def initialize_plugin():
    ...
    try:
        ...
        if collection_name not in collection_names:
            plugin.logger.info(f"Creating vector database collection: {collection_name}")
            ...
            plugin.logger.success(f"Collection {collection_name} created successfully")
        else:
            plugin.logger.info(f"Collection {collection_name} already exists")
    except Exception as e:
        plugin.logger.error(f"Plugin '{plugin.name}' failed to initialize vector database: {e}")
    plugin.logger.success(f"Plugin '{plugin.name}' initialization complete.")
```
...
async def handle_channel_reset(_ctx: AgentCtx):
    plugin.logger.info(f"Plugin '{plugin.name}' received session {_ctx.from_chat_key} reset event.")
    ...
    plugin.logger.info(f"Plugin-specific data for session {_ctx.from_chat_key} has been cleaned up.")
```
...
async def handle_user_message(_ctx: AgentCtx, message: ChatMessage) -> MsgSignal | None:
    ...
    if "forbidden_word" in message.content_text:
        plugin.logger.warning(f"User {_ctx.from_user_id} sent a message containing forbidden words")
...
async def handle_system_message(_ctx: AgentCtx, message: str) -> MsgSignal | None:
    ...
    plugin.logger.info(f"Plugin '{plugin.name}' received admin command: {admin_command}")
...
async def cleanup_plugin():
    plugin.logger.info(f"Plugin '{plugin.name}' is cleaning up...")
    # Example: Close possible database connection pools or clear temporary files
    # await close_database_connections()
    # await clear_temporary_files()
    plugin.logger.success(f"Plugin '{plugin.name}' cleanup complete.")
```
